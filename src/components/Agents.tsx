  ...
  return (
    <section id="agents" className="relative overflow-hidden bg-ink-2">
      ...
      <div className="mx-auto max-w-6xl px-6 py-24 lg:py-32">
        <Reveal>
          <SectionHeading
            kicker="06 — The atelier"
            title="You will not graduate without an agent."
          />
        </Reveal>
        <Reveal delay={80}>
            ...
            <Reveal key={agent.name} delay={i * 70}>
              <article className="group h-full border border-line bg-ink/40 p-8 transition hover:border-gold/40 lg:p-10">
                <div className="flex items-start justify-between gap-4">
                  <h3 className="font-serif text-3xl text-parchment">{agent.name}</h3>
                  <span className="font-mono text-[11px] tracking-[0.2em] text-gold">0{i + 1}</span>
                </div>
                <p className="mt-5 text-[15px] leading-relaxed text-mist">{agent.copy}</p>
                <dl className="mt-8 grid grid-cols-2 gap-4 border-t border-line pt-6 text-sm">
                  <div>
                    <dt className="font-mono text-[10px] tracking-[0.18em] text-mist/70 uppercase">Input</dt>
                    <dd className="mt-1 text-parchment">{agent.input}</dd>
                  </div>
                  <div>
                    <dt className="font-mono text-[10px] tracking-[0.18em] text-mist/70 uppercase">Output</dt>
                    <dd className="mt-1 text-parchment">{agent.output}</dd>
                  </div>
                </dl>
              </article>
            </Reveal>
